...
def _classify_2fe(se, fes, cz_fe, cz_se, dirs, jb_id: int,
    ...
    if jb_id in (2, 5):
        if slab:
            # SE = Decke
            if dir_set <= {"m", "n"}:
                return ("Tv2-1:3",   "Tv2-1:3: 2 Wände treffen Decke(SE) in Mitte")
            if "o" in dir_set:
            ...
            if dir_set == {"o"}:
                # 2 Decken durch Wandmitte → Xv2-13-4
                return ("Xv2-13-4",  "Xv2-13-4: 2 Decken durch Wandmitte (vertikal X)")
            if "o" in dir_set:
...
def run(ifc_path: str, out_dir: str) -> None:
    ...
    # Phase 1: gleiche Element-Mengen → beste strukturelle Perspektive behalten
    seen_keys: dict[frozenset, JunctionResult] = {}

    def structural_score(r: JunctionResult):
        """
        Bewertet, welche SE-Perspektive strukturell korrekter ist.
        Niedriger = besser.
        """

        # 1️⃣ Mehr FEs = informativer
        fe_score = -len(r.fe_ifc_ids)

        # 2️⃣ SE ist Slab → bei gemischten Systemen bevorzugen
        slab_bonus = 0
        if r.se_type == "IfcSlab":
            slab_bonus = -1

        # 3️⃣ X- oder Tv2-* Typen bevorzugen vor Tv1-24
        type_priority = 0
        if r.junction_type.startswith(("X", "Tv2", "Th2")):
            type_priority = -1

        # 4️⃣ Tv1-24 ist generischer → leicht schlechter
        generic_penalty = 1 if r.junction_type in ("Tv1-24", "Th1-24") else 0

        return (generic_penalty, type_priority, slab_bonus, fe_score)
    ...
    for jr in all_junctions:
        ...
        if key not in seen_keys:
            ...
            if structural_score(jr) < structural_score(existing):
                seen_keys[key] = jr
...
def main():
    ...
    # -----------------------------------------------------------------------
    IFC_PATH = "./ifc-models/Tv1-24.ifc"
    OUT_DIR  = "./output"
    # -----------------------------------------------------------------------
